...
type EntityField a = EntityT a (F SqlTimestamptz) (F SqlTimestamptz)
...
withTimestampFields
  ...
  -> EntityT
       ...
       (TableFields (Maybe (F SqlTimestamptz)) (F SqlTimestamptz))
withTimestampFields mapping = Entity
  { record          = mapping
...
withTimestamp = map f
 where
  f r =
    Entity { record = r, recordCreatedAt = Nothing, recordUpdatedAt = Nothing }
