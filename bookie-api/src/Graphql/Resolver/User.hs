...
userResolver user =
  let DB.User {userId, userEmail, userName, userUnitSize} = record user
...
registerResolver RegisterArgs {email, password, name, unitSize} = do
  res :: [DB.User] <- runSelect $ findUserByEmail email
  case res of
    _ : _ -> failRes "This email is already taken"
    [] -> do
      ph <- liftIO $ hashPassword password
      runInsert $ insertUser (email, ph, name, float2Double unitSize)
